...
void CRussia::DrawJiemian1(int a, int b, CDC *pDC)
{
	...
	str.Format("%d",m_Level);
	if(m_Level>=0)
		pDC->TextOut(a+453,b+126,str);
	str.Format("%d",m_Speed); 
	if(m_Speed>=0) 
		pDC->TextOut(a+453,b+64,str);
	str.Format("%d",m_Score); 
	if(m_Score>=0)
		pDC->TextOut(a+453,b+2,str);
	pDC->RestoreDC(nOldDC);
	// DrawScore(pDC);
	//����з��飬��ʾ����
	//��Ϸ��
	...
	for(int n=0; n<4; n++)
		for(int m=0; m<4; m++)
			...
			{ 
				Dc.SelectObject(fangkuai);
				pDC->BitBlt(a+360+m*30, b+240+n*30, 30, 30, &Dc, 0, 0, SRCCOPY);
			}
}
